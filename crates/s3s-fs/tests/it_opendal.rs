...
fn create_operator() -> Operator {
    // opendal 0.58 requires a process-wide HTTP transport to be installed before
    // using HTTP-based services. `auto-register-services` (the ctor-based
    // auto-install) is disabled here via `default-features = false`, so install
    // the reqwest transport explicitly. This call is idempotent and lazy.
