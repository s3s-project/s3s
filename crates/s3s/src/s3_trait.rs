...
pub trait S3: Send + Sync + 'static {
    ...
    /// <p>The POST operation adds an object to a specified bucket using HTML forms. POST is an alternate form of PUT that enables browser-based uploads as a way of putting objects in buckets. Parameters that are passed to PUT via HTTP Headers are instead passed as form fields to POST in the multipart/form-data encoded message body.</p>
    /// <p>For more information about using POST to upload objects, see <a href="https://docs.aws.amazon.com/AmazonS3/latest/API/RESTObjectPOST.html">POST Object</a> in the <i>Amazon S3 API Reference</i>.</p>
    ...
    /// </ul>
    async fn post_object(&self, _req: S3Request<PutObjectInput>) -> S3Result<S3Response<PutObjectOutput>> {
