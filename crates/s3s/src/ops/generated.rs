...
impl PostObject {
    pub fn deserialize_http(req: &mut http::Request) -> S3Result<PutObjectInput> {
        match req.s3ext.multipart.take() {
            Some(m) => Self::deserialize_http_multipart(req, m),
            None => Err(s3_error!(InvalidRequest, "PostObject requires multipart/form-data")),
        }
    }
...
impl super::Operation for PostObject {
    ...
    async fn call(&self, ccx: &CallContext<'_>, req: &mut http::Request) -> S3Result<http::Response> {
        ...
        let s3 = ccx.s3;
        if let Some(access) = ccx.access {
            access.post_object(&mut s3_req).await?;
        }
        let result = s3.post_object(s3_req).await;
        let s3_resp = match result {
            Ok(val) => val,
            Err(err) => return super::serialize_error(err, false),
        };
    ...
    match req.method {
        ...
        hyper::Method::POST => match s3_path {
            ...
            S3Path::Bucket { .. } => {
                ...
                Ok((&PostObject as &'static dyn super::Operation, false))
            }
            S3Path::Object { .. } => {
                if let Some(qs) = qs {
