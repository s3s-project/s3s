...
mod capability_test_helpers {
    ...
    pub struct TestS3WithGetObjectPartNumberCap {
        pub get_object_calls: AtomicUsize,
        pub head_object_calls: AtomicUsize,
    }

    impl TestS3WithGetObjectPartNumberCap {
        pub fn new() -> Self {
            Self {
                get_object_calls: AtomicUsize::new(0),
                head_object_calls: AtomicUsize::new(0),
            }
        }

        pub fn get_object_call_count(&self) -> usize {
            self.get_object_calls.load(Ordering::SeqCst)
        }

        pub fn head_object_call_count(&self) -> usize {
            self.head_object_calls.load(Ordering::SeqCst)
        }
    }

    #[async_trait::async_trait]
    impl crate::s3_trait::S3 for TestS3WithGetObjectPartNumberCap {
        fn capabilities(&self) -> Capabilities {
            Capabilities::empty().with(Capability::GetObjectPartNumber)
        }
        ...
        async fn head_object(
            &self,
            _req: S3Request<crate::dto::HeadObjectInput>,
        ) -> crate::error::S3Result<crate::protocol::S3Response<crate::dto::HeadObjectOutput>> {
            self.head_object_calls.fetch_add(1, Ordering::SeqCst);
            Ok(crate::protocol::S3Response::new(crate::dto::HeadObjectOutput::default()))
        }
    }
}
...
#[tokio::test]
async fn capability_get_object_part_number_rejected_when_undeclared() {
    use crate::config::{S3ConfigProvider, StaticConfigProvider};
    use crate::http::{Body, Request};
    use crate::ops::CallContext;
    use hyper::Method;
    ...
    let mut req = Request::from(
        hyper::Request::builder()
            .method(Method::GET)
            .uri("http://localhost/test-bucket/test-key?partNumber=2")
            .header(crate::header::HOST, "localhost")
            .body(Body::empty())
            .unwrap(),
    );
    ...
    assert_eq!(
        ...
        "GetObject with partNumber must be rejected when backend declares no capability"
    );
    assert_eq!(test_s3.get_call_count(), 0);
}

#[tokio::test]
async fn capability_get_object_without_part_number_accepted_when_undeclared() {
    use crate::config::{S3ConfigProvider, StaticConfigProvider};
    use crate::http::{Body, Request};
    use crate::ops::CallContext;
    use hyper::Method;
    ...
    assert_eq!(test_s3.get_call_count(), 1);
}

#[tokio::test]
async fn capability_get_object_part_number_accepted_when_declared() {
    use crate::config::{S3ConfigProvider, StaticConfigProvider};
    use crate::http::{Body, Request};
    use crate::ops::CallContext;
    use hyper::Method;
    ...
    let test_s3 = Arc::new(capability_test_helpers::TestS3WithGetObjectPartNumberCap::new());
    let s3: Arc<dyn crate::s3_trait::S3> = test_s3.clone();
    let config: Arc<dyn S3ConfigProvider> = Arc::new(StaticConfigProvider::default());
    let ccx = CallContext {
        s3: &s3,
    ...
    let response = super::call(&mut req, &ccx).await.unwrap();
    assert_eq!(response.status, hyper::StatusCode::OK);
    assert_eq!(test_s3.get_object_call_count(), 1);
}

#[tokio::test]
async fn capability_head_object_part_number_rejected_when_undeclared() {
    use crate::config::{S3ConfigProvider, StaticConfigProvider};
    use crate::http::{Body, Request};
    use crate::ops::CallContext;
    use hyper::Method;
    ...
    assert_eq!(
        response.status,
        hyper::StatusCode::NOT_IMPLEMENTED,
        "HeadObject with partNumber must be rejected when backend declares no capability"
    );
}

#[tokio::test]
async fn capability_head_object_part_number_accepted_when_declared() {
    use crate::config::{S3ConfigProvider, StaticConfigProvider};
    use crate::http::{Body, Request};
    use crate::ops::CallContext;
    use hyper::Method;
    ...
    let response = super::call(&mut req, &ccx).await.unwrap();
    assert_eq!(response.status, hyper::StatusCode::OK);
    assert_eq!(test_s3.head_object_call_count(), 1);
}
